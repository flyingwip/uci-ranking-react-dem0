...
$filename = 'uci_ranking.json';


// //current date weeknr
$weeknr_current_date = intval(date("W"));

//echo '<h3>'.$weeknr_current_date.'<h3>';

//When was the last modification of the local file?
if (file_exists($filename)) {
  //echo "$filename was last modified: " . intval(date ("W.", filemtime($filename)));
  $weeknr_file_json =  intval(date ("W.", filemtime($filename)));
}

//Let's see if there is a new ranking list of this week.

//   //set the correct uci phase id
$weeknr_uci = 1428678 + $weeknr_current_date-1;
//   //10 jan = week1 = 1428679;
//   //$phase = 1428681;
//   //17 jan = week1 = 1428680 enz 
...
//Do we get a valid response
if(get_http_response_code($api_url) != "200"){
    //echo "error";

    //not a valid html from UCI server let's work with the current json file;
    $uci_json =  file_get_contents("uci_ranking.json");
    ...
    //is the new 
      ...
      foreach($table->find('tr') as $row) {
          ...
          foreach($row->find('td') as $cell) {
              ...
              if ($counter == 0){
                $temp_array  = $cell->plaintext;
                $temp2 = explode("  ", $temp_array);

                $rank =  trim($temp2[0]);
                $previous =  trim($temp2[1]);
                ...
                //$cc_data[] = $cell->plaintext;
              } elseif ($counter == 1) {

                $cc_data[] =  strtolower($cell->plaintext) ;
      ...
      $blaat = array_shift($rowData);

      //array to JSON
      $uci_json = json_encode($rowData);
      //save to file
      $fp = fopen('uci_ranking.json', 'w');
      fwrite($fp, $uci_json);
      fclose($fp);
